'''
Code which:
1 - Locate and extract sudoku puzzle from image
2 - Locates each cell from the extracted puzzle
3 - Uses Tesseract (OCR) to identify each digit
                ...
                for num in range(1, 10):
                    if is_valid(board, i, j, num):
                        ...
                        if solve_sudoku(board):
                            return True
                        board[i][j] = 0  # Backtrack if the solution was not valid
                return False
    return True
...
def identify_digit(image, debug=False):
     # Resize the image to a fixed size (assuming it's not already)
    ...
    digit_value = pytesseract.image_to_string(image, config='--psm 6 --oem 3').strip()


    # Display the digit and its value for verification
    if debug:
        cv2.imshow("Digit", image)
        print("Identified Digit:", digit_value)
...
# a Sudoku puzzle is a 9x9 grid (81 individual cells), so we can infer the location of each cell by dividing the warped image into a 9x9 grid
